...
def build_boards(g):
 ...
 def add(id,title,sub,body):
  boards.append((id,title,'<section class="board '+('active' if not boards else '')+'" id="'+id+'"><div class="title"><h2>'+title+'</h2><p>旗舰元年 6.9</p></div><p class="subtitle">'+sub+'</p>'+body+'<div class="legend">静态线下展示 / A3横向打印 · 不提供发牌、结算、存局或网络游玩</div></section>'))
 ...
 add('library','公司稳定配件库','四类各一个栏位。初始免费；稳定直接读卡上普通/伙伴入库价。拿到手牌不等于入库。','<div class="four">'+''.join('<div class="cardwell stablewell"><h3>'+n+'</h3><p>实际稳定卡一直放这里</p><p>最多1张<br>新卡替换旧同类<br>不得叠放备用</p><p><b>桌上仍有该类代替卡<br>＝不能换库卡</b></p><p>未来世代可以付费入库<br>但不能被组装或验证样机引用</p></div>' for n in K.values())+'</div>'+p('引用解除，不靠回合或发布自动复位','<p>取消：只收回该批代替卡。成交：全部证据核验与即时奖励后统一收回；之后才做奖励签装／建设。未售：继续引用，跨场不减分。</p><p>其他产品仍有引用，即使只剩一款，该栏也继续锁定。旧稳定被替换进主弃牌，旧初始移出。</p>')+p('回合资源','<div class="slots"><div class="slot">工作1</div><div class="slot">工作2</div><div class="slot">工作3</div><div class="slot">U02专用组装点<br>只有建成启用才有</div></div>'))
 add('product','单批产品面板','每家公司有1—4号四个产品位；此面板可重复打印4份并放产品号片。', '<div class="four">'+''.join('<div class="cardwell"><h3>'+n+'</h3>放“稳定部件·'+n+'”代替卡<br>或实际定制部件</div>' for n in K.values())+'</div><div class="grid2">'+p('部署与可选方案','<div class="slots"><div class="slot">技术1</div><div class="slot">技术2</div><div class="slot">Q方案<br>至多1</div></div><p>部署与配件集成合计上限2；T21仅限原始集成≥1，自占0并抵消集成1；总上限仍2。自主技术留本人完成区；联合留厂区，圆片署名、公司/产品片指派。</p>')+p('付费就是生产','<p>组装1工作 → 检查四类、世代、配套、供电和技术 → 付部件费＋组装费8＋Q费，减免后至少6；外部授权费另付原作者。</p><p><b>现在就是一批待售货，不再另点生产，不叠加库存。</b></p>')+'</div>'+p('取消与结束','<p>本人工作阶段免费取消：定制、Q回手；技术释放，代替卡收回；原款与授权费不退。改款就是取消后重新组装付费。</p><p>成交后定制、Q移出（Q21例外）；未售配置原样保留，不用年龄、旧分或历史成本。</p>'))
 headers=['产品','留库','合同A','合同B（U10）','大众消费','电竞娱乐','摄影创作','商务办公']
 rows=[]
 for i in range(1,5):rows.append([f'{i}号']+['待售\n↓已售\n↓收款']*7)
 add('dest','秘密投放','每批一个产品号去向片，只能放一处。每行一张价牌，入场许可也先锁定，不留奖励券。', '<div class="dest">'+tbl(headers,rows)+'</div>'+p('先锁定，后现抽，再扩产','<p>实际只有4批，就只有4个去向片。合同要2批，必须用两个产品位各交一批；每批分别达标，可不同配置。</p><p>揭晓以后未售不能改投。已售配置和片留到挑战等条件确认并完成所有奖励；不要先拆掉成交产品。</p>')+p('价格速查',tbl(['售价','修正','该批现金','该批收入'],[[18,'+6',18,1],[28,'+2',28,2],[38,'−2',38,3],[48,'−6',48,4]])+'<p>48通常要求基础≥22且某部件规格≥8；其他许可看卡。所有价格仍查预算。</p>'))
 add('research','研究、合同与世代','进度和期限放在实际卡旁；不要记已售型号或它的首次发布。','<div class="grid2">'+p('在研槽：默认2，U09到3',''.join(f'<h4>项目{i}</h4>'+track(8) for i in range(1,4))+'<p>费用与工期逐卡读模式行，不按固定倍率计算。仍只用一枚进度片；立项即1。到线缺证据放待验证；再复核花1点。</p>')+p('执行合同：默认仅A，U10开启B',''.join(f'<h4>合同{x}</h4><div class="slots"><div class="slot">期限1</div><div class="slot">期限2</div><div class="slot">压实际押金</div></div>' for x in 'AB')+'<p>每次发布去1期限；履约拿回押金，逾期押金入银行。</p>')+'</div>'+p('本公司独立验证：当前世代各一次','<div class="slots">'+''.join('<div class="slot">'+e+'<br>已用片永久放这里</div>' for e in ['I','II','III','IV','V'])+'</div><p>工作2、现金12、基础≥10、特征≥2，奖励1科技。T32/T39/U12/Q22例外见卡。取消、换位、成交均不复位。</p>')+p('全桌世代：只改标记，不再加牌','<div class="slots">'+''.join('<div class="slot">'+str(e)+'代<br>科技'+str(t)+'</div>' for e,t in zip(range(1,6),d['parameters']['eraThresholds']))+'</div>'))
 ...
 add('workflow','桌边流程与通用数值','具体费用、工期、订单、星级与奖励都直接读卡；这里打印共用规则。','<div class="grid2">'+p('发布七步','<p>①锁价、选唯一去向，揭晓，再现抽V</p><p>②整单合同核验与交付</p><p>③四市场有限订单逐批分配</p><p>④冻结挑战和卡牌奖励，用片保留证据</p><p>⑤先全桌收款／普通成果，再特色奖</p><p>⑥回购／出口后核验，统一释放已结束产品</p><p>⑦结清签装／建设，再合同维护、刷牌市、换N</p>')+p('产品生命周期','<p><b>组装＝生产1批</b>；先付钱，不能借未来售价。</p><p><b>未售</b>：完整配置留下，不减分，不补制造费。</p><p><b>取消</b>：本人工作阶段，定制/Q回手，钱不退。</p><p><b>成交</b>：定制/Q消耗，部署释放，代替卡收回。</p><p>无年龄、无芯片配额、无公司产能条、无多批库存。</p>')+'</div>'+p('六种特征',tbl(['性能','影像','轻薄','续航','折叠','生态'],[['芯片≥6','影像≥6','机身轻薄','余量≥4','屏/身配套','芯片生态或卡授予']]))+p('即时经营奖励，不留券','<p>渠道回购：12/20/28，另一批合法未售零售货，消耗后释放，不算销售。</p><p>清理后签装：省工作，入库费−3/6/9，未售引用仍锁；建设：省工作，建费−0/3/6，仍需手牌与空间。</p><p>每公司每场一个挑战奖，收入、科技、特色三选一；本场结清，不跨场保存。</p>'))
 add('suppliers','个人供应商关系','每人一份，放本人公司字母。关系不占合同槽；核心全公司只有一家。',p('五厂关系','<div class="suppliericons">'+''.join(g['factory_badge'](f['id']) for f in d['factories'])+'</div>'+tbl(['供应商','普通客户','合作伙伴','核心伙伴'],[[f['name'],'开局关系片','实际业务后升到这里','本公司唯一核心'] for f in d['factories']]))+p('怎样成为伙伴','<p>本厂稳定实际入库；本厂定制实际付费组装；本厂订单/验证正常履约。整项结束后才建立，不能倒算刚付费用。</p><p>初始、拿牌、临时样机、单纯签约、取消/回购/出口、服务到期均不算。</p>')+p('核心1工作＋12现金','<p>已有伙伴才能升。首次核心可并入首次联合：同1工作，付12＋联合费，进度1。以后切换仍另1工作＋12，有未结束联合不可切。多家公司可同厂。</p>')+p('普通/伙伴入库价直接读卡','<p>核心与伙伴同价；不同卡不同入库价，其他合法减免再算到0。地图不另加重复折扣。</p>'))
 add('network','公共供应商技术区','开局为空；仅放完成联合T。卡多时移到对应标题下的桌面继续展开，无格数上限。','<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:4mm">'+''.join('<div class="cardwell" style="height:157mm;border-color:'+f['color']+'"><h3 style="color:'+f['color']+'">'+f['name']+'</h3><p>'+g['factory_icon'](f['id'])+'</p><p>已完成联合科研T</p><p>圆形：永久作者</p><p>长方：A/B/C/D产品指派</p><p>不同公司可同时使用；同公司一项一批</p><p>不补牌、不抽牌、不出售原卡</p></div>' for f in d['factories'])+'</div>'+p('原作者和使用者分别判断','<p>原作者自用免授权费，仍须供应商适配、指派与技术位。外部使用者付作者、得到产品能力，不得到印刷科技或完成项目数。</p>'))
 add('licensing','联合研发与按批授权','先付真实现金，再放指派片。旧批许可跟产品走，不按发布重复收费。','<div class="grid2">'+p('自主 / 联合',tbl(['模式','完成','后续使用'],[['自主','按自主行费用/工期、原证据','仅本人；无需指定供应商'],['联合','按联合行费用/工期、核心与本厂样机','公开厂区；圆署名；同厂关联或明确适配']]))+p('组装付款顺序','<p>①先查本厂伙伴、技术位与关联部件。</p><p>②先备足制造费＋全部外部授权费。</p><p>③制造付银行，授权付对应圆片作者。</p><p>④放公司/产品指派，不额外花授权工作点。</p>')+'</div>'+p('一张T上可以有五个不同用途的片','<p>永久作者圆片A片，表示A完成科研；A①/B③/D②长方片，分别是三家公司的不同产品部署。</p><p>作者没有使用片，就没有给自家产品部署。联合作者换核心不移动老成果；别人合法产品继续用。</p>')+p('从未售到取消或成交','<p>未售保留指派与付款效力；取消回收使用片，不退费用。成交/处置全场证据检查后统一收回使用片，不收圆片。</p><p>临时样机不能免费借别人公开技术；实际已付授权的现货可作证。科技挑战仍须本批有本人完成部署。</p>'))
 ...
 h='<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>旗舰元年6.9线下版图</title><style>'+css+'</style><body><header><h1>旗舰元年6.9</h1>'+nav+'<button onclick="window.print()">打印当前</button><button onclick="printAll()">打印全部</button></header>'+''.join(v for _,_,v in boards)+script+'</body></html>'
 (O/'旗舰元年_6.9_线下版图.html').write_text(h,encoding='utf8');(R/'work/boards_manifest.json').write_text(json.dumps({'views':[{'id':i,'title':t} for i,t,_ in boards]},ensure_ascii=False,indent=2),encoding='utf8')
...
def build_aids(g):
 ...
 def sheet(title,body,copies):
  n=len(sheets)+1;sheets.append(f'<section class="sheet"><div class="head"><b>{title}</b><span>6.9 / {n:02}</span></div>{body}<div class="foot">{copies} · A4原尺寸 · 正反面配对见PRINTING.md</div></section>');manifest.append({'page':n,'title':title,'copies':copies})
 ...
 for i in range(1,5):
  for price,mod,inc in zip(d['parameters']['prices'],d['parameters']['priceScore'],d['parameters']['incomePerBatch']):
   tiles.append(tile(f'{i}号产品 · 售价',f'<div class="big">{price}</div><p>竞争修正 {mod:+}</p><p>原批：{price}现金 / {inc}收入</p><p>原批＋扩产：{price*2}现金 / {inc*2}收入</p><p>48：基础≥22且一部件≥8；卡面另许可除外。预算仍检查。</p>'))
 ...
 tiles=[]
 for i in range(1,5):
  for kind,name in K.items():tiles.append(tile('稳定部件',f'<div class="big">{name}</div><p>{i}号产品专用代替卡</p><p>数值与能力读本人库内{name}。实体留库；只要仍引用，该栏不能替换。</p><p>取消或全场证据核验后的整理收回。</p>'))
 sheet('稳定部件代替卡：4类×4产品＝16张','<div class="items grid4">'+''.join(tiles)+'</div>','每人1份，单面；不混主牌')
 heads=['产品','留库','合同A','合同B（U10）','大众','电竞','摄影','商务']
 rows='<tr>'+''.join('<th>'+x+'</th>' for x in heads)+'</tr>'
 for i in range(1,5):rows+=f'<tr><th>{i}号</th>'+''.join('<td>待售<br>↓<br>已售</td>' for _ in heads[1:])+'</tr>'
 sheet('一批一个去向：放在屏风内','<table class="a-table">'+rows+'</table><div class="notes">每行只放1枚产品号片。该片代表这一整批，不是库存数量。合同B须先建U10。全部扩产结束、最终成交后再向下移；收款后加“已收款”片，仍留配置到全部奖励结束。<br><br>合同需2批＝两行各放一枚到同一合同。无效投放不得现场改去另一处。</div>','每人1份，单面，不用剪开')
 ...
 body=''.join(strip('在研项目 '+str(i)+'（默认2槽，U09开放第3）',[0,1,2,3,4,5,6,7,8,'待验证']) for i in range(1,4))
 body+=''.join(strip('合同'+x+('（默认开放）' if x=='A' else '（U10建成开放）')+'：放押金与期限片',[1,2,'押金']) for x in 'AB')
 ...
 sheet('署名和指派：改用独立组件文件','<div class="notes"><h2>不要使用旧版无公司编号的指派片</h2><p>《供应商署名与授权组件》第1—2页：全桌48枚长方使用片，正面公司＋产品号，背面只有公司。第3页：每公司35枚永久作者圆片。</p><p>A=M01、B=M02、C=M03、D=M04。公司身份与红黄蓝绿黑供应商完全不同。</p><h3>本人的科技</h3><p>自主完成卡在本人完成区；联合完成卡放对应公共厂区并加作者圆片。无论自用还是外部授权，部署都必须在卡上放产品指派片。</p><h3>别人使用</h3><p>组装时付卡面授权费给圆片作者。普通每公司同项技术一批；扩产同组＋1另付一批授权，原指派不改号。未售不重复收费；新的一批重新付。</p><h3>清理</h3><p>长方片随本批取消或最终处置整理收回；圆作者片永远不因此收回。不要复制T卡，不记授权次数，不留授权券。</p></div>','每人1份单面速查；真正可剪指派片见独立文件')
 sheet('供应商关系：经营权限，不是固定技能','<div class="notes"><h2>普通 → 伙伴 → 核心</h2><p>真实安装本厂稳定、付费组装本厂定制或完成订单/验证合同，行为结束后成为伙伴。不追溯刚付费用，不因抽牌或单纯签约建立。</p><p>伙伴可按卡面伙伴入库价安装，也可依法使用本厂公开技术。核心额外可立项联合研发，不给固定技能。</p><p>核心：已有伙伴，1工作＋12；全公司一家。有未结束联合不切换。尚无核心时，首次联合可同1工作付12＋联合费并到1进度，不免费建立伙伴。</p><h2>研究路径</h2><p>两种模式逐卡印费用与工期，不按固定倍数计算；完成科技相同。先选模式行再算优惠。联合仍需核心与本厂样机，公开授权；自主独占。公司工艺仅有自主行。</p><p>只部署技术可联合；工艺只自主。外部授权只提供产品能力，不等于本人完成科研。</p><h2>地图与卡面</h2><p>各卡独立普通/伙伴入库价，差价不固定；不重复扣相同关系优惠。所有卡面删除重复的取得0。取得卡牌仍花原来的工作点。</p></div>','每人1份单面速查；不占合同槽，不新增牌堆，不替代真实授权付款')
 rewards=[tile('渠道回购','<p>1／2／3星：12／20／28现金。</p><p>另一批本场合法零售投放、未成交的货，拿钱并标记回购。清理时定制/Q消耗，引用解除。</p><p>不加收入，不触发挑战或Q21，不再出口同一批。</p>'),tile('供货签装','<p>1／2／3星：入库费减3／6／9。</p><p>全桌统一清理产品后，从手牌安装1张稳定。工作0；余款照付，最低0；仍有引用的栏不能换。</p><p>不是普通安装工作，不触发T36两类许可。</p>'),tile('建设支持','<p>1／2／3星：建设费减0／3／6。</p><p>全桌统一清理后，从手牌建1设施。工作0，现金、空间、前置仍检查。</p><p>后续工作生效，不补领本场收益，不付本场维护。</p>'),tile('每个人开局都有四张真实配件','<p>M01：S011—S014<br>M02：S021—S024<br>M03：S031—S034<br>M04：S041—S044</p><p>芯片、屏幕、影像、机身各1张初始通用实体，免费入四栏。它们不等于代替卡。</p><p>本页是速查，不是卡牌；实体用独立初始牌文件。</p>')]
 sheet('即时奖励与初始发放速查：不是保留券','<div class="items grid2">'+''.join(rewards)+'</div>','每人1份，单面；奖励只在本场结清，不剪成券')
 labels=['产品'+str(i) for i in range(1,5)]+['已收款']*4+['已锁定','挑战选择','转换1科技','已部署']+['本人回合已用']*8+['本期已用']*6+['维护停用','待验证']*2+['集成占位']*4+['回购处置']*2+['出口处置']*2+['验证已用']*5+['工作1','工作2','工作3','U02专用']+['＋100收入','＋36科技\n成绩＋72']
 ...
 work=g['table'](['工作','点数'],[['取主牌至多2',1],['安装稳定1张',1],['组装1批',1],['推进/复核研发',1],['签约合同',1],['设施建设',2],['预热+1—3',1],['技术服务6/10',1],['独立验证12现金',2],['核心合作12现金',1],['取消未成交产品',0]])
 summary=[tile('工作速查',work+'<p>预热与服务每本人回合各一次。每回合3点，不保留余点。</p>'),tile('组装费用与库存','<p>制造：四部件＋组装费8＋Q费，合法减免后至少6；外部授权另付作者，不参与此最低值或制造减免。</p><p>取得主牌不另付现金。稳定读普通/伙伴入库价；初始开局免费。</p><p>普通每位1批；本场扩产可＋1，不另开位或留复制库存。</p><p>未来配件可拿可入库，不可组装。</p>'),tile('特征与市场','<p>性能：芯片规格≥6<br>影像：影像规格≥6<br>轻薄：机身轻薄<br>续航：供电余量≥4<br>折叠：屏幕机身配套<br>生态：芯片标识或能力</p><p>大众任意合法；电竞性能；摄影影像；商务续航/生态/折叠。</p>'),tile('取消与成交','<p>本人工作阶段可免费取消：定制与Q回手、T释放、代替卡收回；钱和点不退。</p><p>成交结束：定制与Q移出（Q21例外）；未售原样跨场，不扣年龄。</p><p>有任何同类代替卡引用就不能换该库栏。通常发布中不能换库；供货签装仅在统一释放后例外安装。</p>')]
 ...
 h=g['document_html']('旗舰元年 6.9 · 实体组件',AIDCSS,''.join(sheets))
 (O/'旗舰元年_6.9_实体组件.html').write_text(h,encoding='utf8');(R/'work/aids_manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf8')
...
def build_reference_print(g):
 ...
 for era in range(1,6):
  ...
  back='<article class="refcard back"><p>FLAGSHIP ERA / 6.9</p><h3>参考世代 '+str(era)+'</h3><p>全部锁定后才抽取<br>六张完整洗匀，允许重复</p></article>'
  for side,body in [('正面',''.join(front)),('背面',back*6)]:
   page=len(sheets)+1;sheets.append(f'<section class="sheet"><div class="head"><b>参考{era}代 · {side}</b><span>6.9 / {page}</span></div><div class="items refgrid">'+body+'</div><div class="foot">全桌1套 · 前后页长边翻转 · 同一代背面完全一致 · 原尺寸打印</div></section>');manifest.append({'page':page,'era':era,'side':side,'cards':[c['id'] for c in cards] if side=='正面' else []})
 (O/'旗舰元年_6.9_参考机正反面.html').write_text(g['document_html']('旗舰元年6.9 · 参考机正反面',AIDCSS,''.join(sheets)),encoding='utf8');(R/'work/reference_print_manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf8')
...
def build_main_print(g):
 ...
 for j in range(0,len(expanded),9):
  ...
  backs=['<div class="main-back"><h3>旗舰元年</h3><p>主牌 / 6.9</p><p>共同发布 · 单批产品</p></div>' if i<len(chunk) else '<div class="blank">空位</div>' for i in range(9)]
  mirrored=[]
  for row in range(3):mirrored+=backs[row*3:row*3+3][::-1]
  for side,body in [('正面',front),('背面',mirrored)]:
   page=len(sheets)+1;sheets.append(f'<section class="sheet"><div class="head"><b>统一主牌制卡 · {side}</b><span>63×88mm / 6.9 / {page}</span></div><div class="main-grid">'+''.join(body)+'</div><div class="foot">9卡/页 · 原尺寸A4 · 相邻页长边翻转双面 · 建议不透明牌套 · 无出血量产刀模</div></section>')
 (O/'旗舰元年_6.9_统一主牌正反面.html').write_text(g['document_html']('旗舰元年 6.9 · 257张统一主牌正反面',css,''.join(sheets)),encoding='utf8');(R/'work/main_print_manifest.json').write_text(json.dumps({'cards':manifest,'pages':len(sheets),'cardSizeMm':[63,88]},ensure_ascii=False,indent=2),encoding='utf8')
...
def build_starter_print(g):
 ...
 for j in range(0,len(cards),9):
  ...
  for c in chunk:
   ...
   text+=g['table'](['入库费','入库工作','每批制造'],[[0,0,a['batchCost']]])
   ...
   backs.append('<article class="startercard back"><h3>旗舰元年</h3><p>6.9 / 初始通用配件</p><h3>'+company+'</h3><p>'+name+' · 真实供货实体</p><p>不混主牌，不是稳定代替卡</p></article>')
  ...
  for side,items in [('正面',front),('背面',mirror)]:
   page=len(pages)+1;pages.append(f'<section class="sheet"><div class="head"><b>初始通用配件 · {side}</b><span>6.9 / {page}</span></div><div class="items startergrid">'+''.join(items)+'</div><div class="foot">全桌印一套16张 · 63×88mm · 相邻页长边翻转 · 各玩家取本人编号四张 · 不混入主牌</div></section>')
 (O/'旗舰元年_6.9_初始通用配件正反面.html').write_text(g['document_html']('旗舰元年 6.9 · 初始通用配件16张',css,''.join(pages)),encoding='utf8')
