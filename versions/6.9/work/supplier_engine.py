...
class Network:
    def __init__(self,data,players='ABCD'):
        ...
        self.players={n:Player(n) for n in players};self.public={};self.phase='work';self.era=1
        self.removed=[];self.discarded=[];self.bank_paid=0
    def working(self):
        if self.phase!='work':raise ValueError('只能在正常工作阶段处理')
    ...
    def install(self,name,cid,discount=0,*,reward=False):
        ...
        points=0 if reward else 1
        if p.cash<fee or p.work<points:raise ValueError('现金/工作不足')
        p.cash-=fee;p.work-=points;self.bank_paid+=fee;p.hand.remove(cid)
    ...
    def core_cooperate(self,name,factory):
        self.working();p=self.players[name]
        ...
        if p.cash<12 or p.work<1:raise ValueError('费用不足')
        p.cash-=12;p.work-=1;self.bank_paid+=12;p.core=factory
    ...
    def start(self,name,tid,joint=False,discount=0,slots=2,*,first_core=None):
        self.working();p=self.players[name];t=self.techs[tid]
        ...
        fee=max(6,(t['jointCost'] if joint else t['cost'])-discount)
        total=fee+core_fee
        if p.cash<total or p.work<1:raise ValueError('费用不足')
        p.cash-=total;p.work-=1;self.bank_paid+=total;p.hand.remove(tid)
        if first_core is not None:p.core=first_core
        p.research[tid]=Research(tid,name,factory,1);return total
    def advance(self,name,tid,*,original_proof=False,prototype_cards=None,extra_progress=0):
        self.working();p=self.players[name];r=p.research[tid];t=self.techs[tid]
        if p.work<1:raise ValueError('工作不足')
        target=t['jointWork'] if r.factory is not None else t['selfWork']
        if extra_progress<0:raise ValueError('推进不能为负')
        p.work-=1;r.progress=min(target,r.progress+1+extra_progress)
        if r.progress<target or not original_proof:return False
        if r.factory:
            if not prototype_cards:return False
            try:evaluate(prototype_cards,era=self.era)
            except ValueError:return False
            if not any(c.get('supplier')==r.factory and any(a['kind']==t['relatedPart'] for a in c['components']) for c in prototype_cards):return False
    ...
    def assemble(self,name,slot,stable=(),custom=(),techs=(),scheme=None,*,total_discount=0):
        ...
        if p.work<1 or p.cash<cost+sum(payments.values()):raise ValueError('制造和全部授权必须预付')
        # All guards passed: atomic payment and pointers. Triggered card rewards are outside this finite model.
        p.work-=1;p.cash-=cost+sum(payments.values());self.bank_paid+=cost
