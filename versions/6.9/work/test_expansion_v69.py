...
R=Path(__file__).resolve().parents[1];D=json.loads((R/'work/v69_data.json').read_text())
...
class ExpansionTests(unittest.TestCase):
 ...
 def test_expansion_still_uses_current_printed_chip_costs(self):
  before=json.loads((R/'source/v68_data.json').read_text())
...
if __name__=='__main__':
 ...
 report={'version':'6.9','testsRun':result.testsRun,'failures':len(result.failures),'errors':len(result.errors),'multiMarketScenarios':1200,'success':result.wasSuccessful(),'limitations':['输入的完整配置合法性、锁定竞争力和卡面适用范围由调用者预先核验','有限成本模型不代替全部合同/设施/方案解释器','情境不是整局、不是胜率或真人实测']}
 (R/'work/expansion_test_results_v69.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
