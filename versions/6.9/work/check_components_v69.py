...
R=Path(__file__).resolve().parents[1];d=json.loads((R/'work/v69_data.json').read_text());ts={c['id']:c for c in d['technologies']}
...
report={'version':'6.9','partWitnesses':len(witnesses),'searchAttempts':attempts,'assumptions':['所选配件实际可取得，芯片和技术先验已可用','所列技术均由本人自主完成，不需外部授权','工作、资金、产品位足够；不保证实际牌流或科技前置取得路径'],'limits':['只验证配件数值/兼容与部分研发修正','不穷举价格、偏好、Q/H/U或全部供应关系联动'],'witnesses':witnesses}
(R/'work/component_witnesses_v69.json').write_text(json.dumps(report,ensure_ascii=False,indent=2));print('part witnesses',len(witnesses),'attempts',attempts)
