...
with sync_playwright() as p:
 ...
 for f in (R/'outputs').glob('*.html'):
  ...
  issues=page.evaluate('''() => {
    let out=[];for(let sel of ['.card','.refcard','.sheet','.page','.board','.tile']){
      for(let e of document.querySelectorAll(sel)){
       let r=e.getBoundingClientRect();if(!r.height)continue;
       let foot=e.querySelector(':scope > .bottom,:scope > .pagefoot,:scope > .foot,:scope > .legend');let limit=foot?foot.getBoundingClientRect().top:r.bottom;
  ...
  if '全卡牌图鉴' in f.name:
   for id in ['C03','X29','T36','H01','Q24','V15']:page.locator('#'+id).screenshot(path=str(qa/f'card_{id}.png'))
  if '实体组件' in f.name:
