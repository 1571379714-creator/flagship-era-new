...
book=json.loads((R/'work/v62_rulebook.json').read_text(encoding='utf8'))
...
def make_rulebook():
 ...
 p=sec.header.paragraphs[0];p.text='旗舰元年  /  完整规则书                                         6.2  统一牌流与单批产品'
 for run in p.runs:run.font.size=Pt(8);run.font.color.rgb=RGBColor.from_string('57707A')
 p=sec.footer.paragraphs[0];p.alignment=WD_ALIGN_PARAGRAPH.RIGHT;p.add_run('FLAGSHIP ERA  /  6.2   ·   ').font.size=Pt(8);p._p.append(xt('fldSimple',{'instr':'PAGE'}))
 doc.add_paragraph('FLAGSHIP ERA  /  6.2',style='Subtitle');doc.add_paragraph('旗舰元年',style='Title');doc.add_paragraph('完整规则书',style='Title');doc.add_paragraph('统一牌流与单批产品',style='Subtitle')
 doc.add_paragraph('\n')
 ...
 doc.add_paragraph('\n2—4人  ·  243种设计 / 315张内容牌\n2026年9月22日  ·  测试参数，尚未完成充分多人实测。')
 doc.add_page_break()
 doc.add_paragraph('阅读导航',style='Heading 1')
 doc.add_paragraph('先看§1—3认识资源和回合。第一次造机查§4—7；研究合同查§8—9；发布查§10—12。卡牌的完整数值另见图鉴。')
 ...
 for s in book['sections']:
  ...
  if s['intro']:doc.add_paragraph(s['intro'])
  for b in s['blocks']:
 ...
 doc.core_properties.title='旗舰元年6.2 完整规则书';doc.core_properties.author='';doc.core_properties.subject='正文与侧栏 / 隐藏参考 / 实体操作'
 doc.save(O/'旗舰元年_6.2_完整规则书.docx')
 ...
 for s in book['sections']:
  ...
  body+='</section>'
 (O/'旗舰元年_6.2_完整规则书.html').write_text('<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>旗舰元年6.2 易读规则</title><style>'+css+'</style><header><h1>旗舰元年 6.2</h1><p>完整规则书 · 统一牌流与单批产品</p><button onclick="window.print()">打印</button></header><nav>'+nav+'</nav><main>'+body+'</main></html>',encoding='utf8')
