...
d=json.loads((R/'work/v62_data.json').read_text(encoding='utf8')); E=lambda x:html.escape(str(x))
M={x['id']:x['name'] for x in d['markets']};F={x['id']:x for x in d['factories']}
K={'chip':'芯片','screen':'屏幕','camera':'影像','body':'机身'}
N={'supply':'稳定配件','custom':'定制配件','starter':'初始稳定件','technology':'研发项目','contract':'合同','facility':'设施','scheme':'产品方案','benchmark':'参考机','demand':'需求'}
...
def card_body(c):
 ...
 if t in ('supply','custom','starter'):
  s='<div class="metrics">'+metric('世代','初始' if t=='starter' else c['era'])+metric('取得',0)
  if t!='custom':s+=metric('入库',c['installCost'])
  s+='</div>'+table(['部件 / 形态','制造费','规格','耗 / 供','标识'],[[K[a['kind']]+' / '+('折叠' if a['form']=='fold' else '直板'),a['batchCost'],a['spec'],('供' if a['kind']=='body' else '耗')+str(a['power']),a['mark']] for a in c['components']])
  s+='<p class="effect">'+E(c['effect'])+'</p><p class="small">'+E(c['lifecycle'])+'</p>'
  hints=[]
  kinds={a['kind'] for a in c['components']}
  if 'chip' in kinds:hints.append('性能：芯片规格≥6；生态：芯片生态标识。')
  ...
  s='<div class="metrics">'+metric('研发费',c['cost'])+metric('工期',c['work'])+metric('科技',c['gain'])+'</div>'
  s+=f'<p class="small">领域：{E(c["field"])} · 此前完成项目≥{c["prerequisiteProjects"]} · 世代{c["era"]}</p><p><b>完成证据：</b>{E(c["proof"])}</p><p class="effect">{E(c["effect"])}</p>'
 elif t=='contract':
  f=F[c['factory']];s=f'<span class="factory" style="--fc:{f["color"]}">{E(f["name"])} · {E(f["shape"])}</span>'
  ...
  s='<div class="metrics">'+metric('建设费',c['cost'])+metric('工作',c['work'])+metric('空间',c['space'])+'</div><p class="effect">'+E(c['effect'])+'</p><p class="small">先从主牌取得，再付工作与现金建设。限次片放本卡；取消产品不复位。</p>'
 elif t=='scheme':
...
def render_card(c):
 ...
 if c['type']=='technology':foot+=' · '+('公司工艺' if c['scope']=='process' else '每批通常2技术位')
 return f'<article class="card" id="{c["id"]}" style="--accent:{C[c["type"]]}"><div class="eyebrow"><span>{c["id"]} / {N[c["type"]]}</span><span>{str(c["era"])+"代 · " if c.get("deck")=="main" else ""}实体 ×{c["qty"]}</span></div><h3>{E(c["name"])}</h3>{card_body(c)}<div class="bottom"><span>{foot}</span><span>6.2</span></div></article>'
...
def build_catalog():
 groups=[('稳定配件',[c for c in d['parts'] if c['type']=='supply'],6),('定制配件',[c for c in d['parts'] if c['type']=='custom'],4),('初始稳定件',[c for c in d['parts'] if c['type']=='starter'],6),('研发项目',d['technologies'],6),('经营合同',d['contracts'],6),('设施建设',d['facilities'],6),('产品方案',d['schemes'],6),('参考机',d['benchmarks'],4),('需求牌',d['demands'],4)]
 pages=[];manifest=[];ranges=[];page=3
 for title,cards,n in groups:
  start=page
  for i in range(0,len(cards),n):
   cs=cards[i:i+n];pages.append(f'<section class="page"><div class="pagehead"><b>{title}</b><span>6.2 / 完整图鉴</span></div><div class="cardgrid {"tall" if n==4 else ""}">'+''.join(render_card(c) for c in cs)+f'</div><div class="pagefoot"><span>统一牌流 · 单批产品 · 不能混用6.1旧效果</span><span>{page}</span></div></section>')
 ...
 cover='<section class="page cover"><p>FLAGSHIP ERA / 6.2</p><div class="big">旗舰元年</div><div class="sub">完整卡牌图鉴</div><div class="lede">取得免费，稳定入库付费。<br>四栏供货，代替卡引用；一组配置就是一批现货。<br>读卡上的数值，做桌上的生意。</div><div class="statrow"><div><strong>243</strong>种设计</div><div><strong>315</strong>张内容牌</div><div><strong>257</strong>张主牌</div></div><p class="watermark">完整卡面参考稿 · 重复按×份数准备<br>本图鉴按种展示；另附257张统一主牌正反面。<br>不含量产出血刀模；功能组件另册。<br>2026年9月22日 · 未充分多人实测</p></section>'
 index='<section class="page index"><h2>内容索引</h2>'+table(['类别','设计数','页码'],[[x[0],x[1],f'{x[2]}—{x[3]}'] for x in ranges])+'<div class="note">主牌：稳定80、定制64、研发41、合同30、设施18、方案24，共257张。五代开局全部混洗。<br>取得费用全部为0；稳定的入库9另付，每批制造费照卡。<br>研发的工期与科技奖励不是同一数字。合同期限按发布，不按玩家回合。<br>稳定实体不离库；取消回收定制和Q，成交消耗；未售保留。</div><p>需求的订单量直接印2/3/4人列；参考每格直接印收入、科技和特色值。通用数值见实体工作速查卡。</p></section>'
 (O/'旗舰元年_6.2_全卡牌图鉴.html').write_text(document_html('旗舰元年 6.2 · 全卡牌图鉴',CSS,cover+index+''.join(pages)),encoding='utf8')
 ...
 text=['# 旗舰元年6.2 完整文字图鉴','243种设计 / 315张内容牌；来源work/v62_data.json。']
 ...
 (R/'work/v62_catalog.md').write_text('\n\n'.join(text)+'\n',encoding='utf8')
...
def build_planning():
 ...
 for n,size,b in [('Normal',10.5,False),('Title',29,True),('Heading 1',17,True),('Heading 2',12,True)]:
  st=doc.styles[n];st.font.name='Noto Sans CJK SC';st._element.get_or_add_rPr().rFonts.set(qn('w:eastAsia'),st.font.name);st.font.size=Pt(size);st.font.bold=b;st.paragraph_format.space_after=Pt(6);st.paragraph_format.line_spacing=1.17
  if n.startswith('Heading'):st.font.color.rgb=RGBColor.from_string('146A76');st.paragraph_format.keep_with_next=True
 s.header.paragraphs[0].text='旗舰元年 / 6.2 设计规划书';s.header.paragraphs[0].runs[0].font.size=Pt(8)
 p=s.footer.paragraphs[0];p.alignment=WD_ALIGN_PARAGRAPH.RIGHT;p.add_run('6.2 / ');p._p.append(xt('fldSimple',{'instr':'PAGE'}))
 doc.add_paragraph('旗舰元年',style='Title');doc.add_paragraph('6.2 设计规划书',style='Title');doc.add_paragraph('统一牌流与单批产品 · 从6.1延续，不回到5.x。')
 text=(R/'work/v62_planning.md').read_text(encoding='utf8')
 ...
 doc.core_properties.author='';doc.core_properties.title='旗舰元年6.2设计规划书';doc.save(O/'旗舰元年_6.2_设计规划书.docx')

# Native HTML layout boards and reusable functional cards; no game state functions.
from physical_layout import build_boards,build_aids,build_reference_print,build_main_print

def export_pdf():
 lo=shutil.which('libreoffice') or shutil.which('soffice')
 if not lo:raise RuntimeError('需要LibreOffice导出规则书与规划书PDF')
 for f in O.glob('*.docx'):
  profile=Path(tempfile.mkdtemp(prefix='flagship62-lo-')).as_uri()
...
if __name__=='__main__':
 ...
 check();make_rulebook();build_planning();build_catalog();build_boards(globals());build_aids(globals());build_reference_print(globals());build_main_print(globals())
 if not args.no_pdf:export_pdf()
 print('Built 6.2 actual files; no migration or remote write.')
