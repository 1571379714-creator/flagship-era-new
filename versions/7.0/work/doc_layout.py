...
book=json.loads((R/'work/v70_rulebook.json').read_text(encoding='utf8'))
...
def make_rulebook():
 ...
 p=sec.header.paragraphs[0];p.text='旗舰元年  /  完整规则书                                         7.0  双行排程与世代竞逐'
 for run in p.runs:run.font.size=Pt(8);run.font.color.rgb=RGBColor.from_string('57707A')
 p=sec.footer.paragraphs[0];p.alignment=WD_ALIGN_PARAGRAPH.RIGHT;p.add_run('FLAGSHIP ERA  /  7.0   ·   ').font.size=Pt(8);p._p.append(xt('fldSimple',{'instr':'PAGE'}))
 doc.add_paragraph('FLAGSHIP ERA  /  7.0',style='Subtitle');doc.add_paragraph('旗舰元年',style='Title');doc.add_paragraph('完整规则书',style='Title');doc.add_paragraph('双行排程与世代竞逐',style='Subtitle')
 doc.add_paragraph('\n')
 ...
 doc.add_paragraph('四个容易混淆的词',style='Heading 2');doc.add_paragraph('产品位＝一批现货；稳定库＝共享供货。本人回合＝选择一张行动并整理阵列；准备期＝两次实际发布之间。')
 doc.add_page_break()
 ...
 doc.core_properties.title='旗舰元年7.0 完整规则书';doc.core_properties.author='';doc.core_properties.subject='正文与侧栏 / 隐藏参考 / 实体操作'
 doc.save(O/'旗舰元年_7.0_完整规则书.docx')
 ...
 for s in book['sections']:
  ...
  body+='</section>'
 (O/'旗舰元年_7.0_完整规则书.html').write_text('<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>旗舰元年7.0 易读规则</title><style>'+css+'</style><header><h1>旗舰元年 7.0</h1><p>完整规则书 · 双行排程与世代竞逐</p><button onclick="window.print()">打印</button></header><nav>'+nav+'</nav><main>'+body+'</main></html>',encoding='utf8')
