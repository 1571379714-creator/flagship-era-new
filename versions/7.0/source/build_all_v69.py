...
d=json.loads((R/'work/v69_data.json').read_text(encoding='utf8')); E=lambda x:html.escape(str(x))
...
def render_card(c):
 ...
 return f'<article class="card" id="{c["id"]}" data-type="{c['type']}" style="--accent:{C[c["type"]]}"><div class="eyebrow"><span>{c["id"]} / {N[c["type"]]}</span><span>{str(c["era"])+"代 · " if c.get("deck")=="main" else ""}实体 ×{c["qty"]}</span></div><h3>{E(c["name"])}</h3>{card_body(c)}<div class="bottom"><span>{foot}</span><span>6.9</span></div></article>'
...
def build_catalog():
 ...
 for title,cards,n in groups:
  ...
  for i in range(0,len(cards),n):
   cs=cards[i:i+n];pages.append(f'<section class="page"><div class="pagehead"><b>{title}</b><span>6.9 / 完整图鉴</span></div><div class="cardgrid {"tall" if n==4 else ""}">'+''.join(render_card(c) for c in cs)+f'</div><div class="pagefoot"><span>统一牌流 · 单批产品 · 供应商网络 · 技术署名与按批授权 · 扩产接口请使用6.9卡文</span><span>{page}</span></div></section>')
 ...
 (O/'旗舰元年_6.9_全卡牌图鉴.html').write_text(document_html('旗舰元年 6.9 · 全卡牌图鉴',CSS,cover+index+''.join(pages)),encoding='utf8')
 ...
 text=['# 旗舰元年6.9 完整文字图鉴','315种设计 / 315张内容牌；来源work/v69_data.json。']
 ...
 (R/'work/v69_catalog.md').write_text('\n\n'.join(text)+'\n',encoding='utf8')
...
def build_planning():
 ...
 s.header.paragraphs[0].text='旗舰元年 / 6.9 设计规划书';s.header.paragraphs[0].runs[0].font.size=Pt(8)
 p=s.footer.paragraphs[0];p.alignment=WD_ALIGN_PARAGRAPH.RIGHT;p.add_run('6.9 / ');p._p.append(xt('fldSimple',{'instr':'PAGE'}))
 doc.add_paragraph('6.9 五十平台设计规划',style='Title');doc.add_paragraph('旗舰元年 · 从6.8定向更新 · 完整名单、真实命名与工程专长')
 text=(R/'work/v69_planning.md').read_text(encoding='utf8')
 ...
 doc.core_properties.author='';doc.core_properties.title='旗舰元年6.9设计规划书';doc.save(O/'旗舰元年_6.9_设计规划书.docx')

# Native HTML layout boards and reusable functional cards; no game state functions.
from physical_layout import build_boards,build_aids,build_reference_print,build_main_print,build_starter_print
from supplier_layout import build_supplier_aids
from expansion_layout import build_expansion_aids,integrate_expansion_surfaces

def export_pdf():
 lo=shutil.which('libreoffice') or shutil.which('soffice')
 if not lo:raise RuntimeError('需要LibreOffice导出规则书与规划书PDF')
 for f in O.glob('*.docx'):
  profile=Path(tempfile.mkdtemp(prefix='flagship69-lo-')).as_uri()
...
if __name__=='__main__':
 ...
 check();make_rulebook();build_planning();build_catalog();build_boards(globals());build_aids(globals());build_reference_print(globals());build_main_print(globals());build_starter_print(globals());build_supplier_aids(globals());build_expansion_aids(globals());integrate_expansion_surfaces(globals());__import__('build_chip_report').build_chip_report()
 if not args.no_pdf:export_pdf()
 print('Built 6.9 actual files; no migration or remote write.')
